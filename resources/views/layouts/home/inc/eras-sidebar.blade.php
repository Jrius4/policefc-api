                ...
                <aside class="tm-sidebar-a uk-width-medium-1-4 uk-pull-3-4 uk-row-first">
                        ...
                        <div>
                            <ul class="nav menu">
                                @foreach ($categories as $category)
                                    <li class="item-3">             
                                    <a href="{{route('eras.era-category',$category->slug)}}">
                                            @if ($category->eras!=null)
                                            {{ $category->title }} <span class="label">({{ $category->eras->count() }})</span>
                                            @else
                                                {{null}}
                                            @endif
                    ...
                    <div class="uk-panel news-sidebar">
                        <h3 class="uk-panel-title">Most Early Eras</h3>
                                    ...
                                    <div class="widget-body">
                                        <ul class="categories">
                                            @foreach($archives as $archive)
                                                <li>
                                                    <a href="{{ route('eras', ['month' => $archive->month, 'year' => $archive->year]) }}">{{ month_name($archive->month) . " " . $archive->year }}</a>
